...
class Bulk {
    ...
    public function __construct() {
        ...
        $this->password = $setting->bulk_password;
    ...
    public function attendance_sms($username, $password, $mobile, $message, $sender, $type) {
        $pingurl = "skycon.bulksms5.com/sendmessage.php";

        $ch = curl_init();
        curl_setopt($ch, CURLOPT_URL, $pingurl);
        curl_setopt($ch, CURLOPT_POST, 1);
        curl_setopt($ch, CURLOPT_POSTFIELDS, 'user=' . $username . '&password=' . $password . '&mobile=' . $mobile . '&message=' . urlencode($message) . '&sender=' . $sender . '&type=3');
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
        $result = curl_exec($ch);

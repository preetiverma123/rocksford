...
class Setting extends MY_Controller {
    ...
    * ********************************************************** */
    ...
    private function _upload_signature() {
        ...
        $prevoius_signature = @$_POST['signature_prev'];
        ...
        if ($signature_name != "") {
            if ($signature_type == 'image/jpeg' || $signature_type == 'image/pjpeg' ||
                ...
                $destination = 'assets/uploads/signature/';

                $file_type = explode(".", $signature_name);
                $extension = strtolower($file_type[count($file_type) - 1]);
                $signature_path = time().'-school-signature.' . $extension;

                copy($_FILES['signature']['tmp_name'], $destination . $signature_path);

                if ($prevoius_signature != "") {
                    // need to unlink previous image
                    if (file_exists($destination . $prevoius_signature)) {
                        @unlink($destination . $prevoius_signature);
                    }
                }

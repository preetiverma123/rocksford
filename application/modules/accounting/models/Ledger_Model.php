...
class Ledger_Model extends MY_Model {
    ...
    public function get_ledger_list_where($reg_no){
        
        $this->db->select('I.*, IN.custom_invoice_id,SEC.name as section_name, S.name AS student_name,E.*, AY.session_year, C.name AS class_name');
        $this->db->from('ledger AS I');        
        $this->db->join('invoices AS IN', 'IN.id = I.invoice_id', 'left');
        $this->db->join('classes AS C', 'C.id = I.class_id', 'left');
        $this->db->join('students AS S', 'S.id = I.student_id', 'left');
        $this->db->join('academic_years AS AY', 'AY.id = I.academic_year_id', 'left');
        $this->db->join('enrollments AS E', 'E.student_id = I.student_id', 'left');
        $this->db->join('sections AS SEC', 'SEC.id = E.section_id', 'left');
        $this->db->where('I.invoice_type', 'academic'); 
        ...
        */
        if($reg_no){
            $this->db->where('I.registration_no =', $reg_no);  
        }
        if($this->session->userdata('role_id') == STUDENT){
            $this->db->where('I.student_id', $this->session->userdata('profile_id'));
        }
       ...
       // $this->db->get_where('invoices', $index_array);
        return $this->db->get()->result();        
    }

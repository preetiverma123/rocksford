            ...
            if(!empty($students)){
                ...
                foreach($students as $key => $obj){
                    ...
                    ////Calculate Previous Dues
                        ...
                        $this->db->select('*');
                        $this->db->from('ledger');
                        $this->db->where('registration_no',$this->data['st'][$key]['student_details']->registration_no);
                        $this->db->order_by('created_at','desc');
                        $query = $this->db->get();
                        ...
                        if(!empty($ledger_prev[0]->balance)){  
                            $this->data['st'][$key]['previous_dues'] =  $ledger_prev[0]->balance;
                        }
                    //////End    
                }
            }

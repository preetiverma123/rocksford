...
class Payment extends My_Controller {
            ...
            if ($this->form_validation->run() === TRUE) {
                ...
                if($this->input->post('payment_method') == 'cash' || $this->input->post('payment_method') == 'cheque'){
                    ...
                    if($this->input->post('amount') < $this->input->post('due_amount')){
                        ...
                        $status = $this->send_message($data,$this->input->post('amount'), $this->input->post('due_amount'));
                    ...
                    $this->db->where('id', $invoiceDetails->student_id);
                    $this->db->order_by('created_at','desc');
                    $query = $this->db->get();
                    ...
                    $ledger['student_id'] = $ledger_prev[0]->student_id;
                    $ledger['class_id'] = $ledger_prev[0]->class_id;
                    ...
                    $ledger['registration_no'] =  $ledger_prev[0]->registration_no;
                    $ledger['fee_due'] = 0.0;
                    /*if($ledger_prev[0]->fee_due==0.0){*/
                        $ledger['previous_due'] = $ledger_prev[0]->balance;
                   /* }else{
                        $ledger['previous_due'] = $ledger_prev[0]->fee_due;
                    }*/
                    
                    $ledger['total'] = $ledger['fee_due'] + $ledger['previous_due'];
                    $ledger['payment'] = $this->input->post('amount');
                    $ledger['date'] = date('Y-m-d');
                    $ledger['balance'] = $ledger_prev[0]->balance - $this->input->post('amount');
                    $ledger['month'] =  $ledger_prev[0]->month;
                    $ledger['created_at'] = date('Y-m-d H:i:s');
                   
                    $updated = $this->ledger->insert('ledger', $ledger);
                    /////End                    
    ...
    * ********************************************************** */
    ...
    /* Paypal payment end */
     ...
     public function send_message($data,$amount,$due_amount) {
...
*/
            ...
            $this->db->select('a.id,a.student_id,b.id,b.guardian_id,b.name,c.id,c.phone,c.name as guardian_name');
            $this->db->from('invoices a'); 
            $this->db->join('students b', 'b.id=a.student_id', 'left');
            $this->db->join('guardians c', 'c.id=b.guardian_id', 'left');
            $this->db->where('a.id',$data['invoice_id']);        
            $query = $this->db->get(); 
            if($query->num_rows() != 0)
            {
                $details = $query->result_array();
                if(!empty($details[0]['phone'])){
                  
                    $message="Hello, Mr. ".$details[0]['guardian_name']." .Your ward ".$details[0]['name']." has submitted: Rs".$amount." and his due amount is :Rs".$due_amount." on ".date('Y-m-d');
           
                    $url="skycon.bulksms5.com/sendmessage.php?user=".urlencode($username)."&password=".urlencode($password)."
                    &mobile=".urlencode($details[0]['phone'])."&message=".urlencode($message)."&sender=".urlencode($sender)."&type=3";
                    ...
                    curl_setopt($ch, CURLOPT_POSTFIELDS, 'user=' . $username . '&password=' . $password . '&mobile=' . $details[0]['phone'] . '&message=' . urlencode($message) . '&sender=' . $sender . '&type=3');

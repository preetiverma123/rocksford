            ...
            if ($this->form_validation->run() === TRUE) {
                ...
                if ($insert_id) {
                    $this->__insert_enrollment($insert_id);
                     if(!empty($this->input->post('previous_due'))){
                        /////create invoice
                        $data = array();
                        ...
                        // save invoice data
                        $data['custom_invoice_id'] = $this->student->get_custom_id('invoices', 'INV');
                        $data['student_id'] = $student_id;
                        $data['class_id'] = $class_id;
                        $data['discount'] = $discount;
                        $data['gross_amount'] = $this->input->post('previous_due');
                        $data['net_amount'] = $this->input->post('previous_due');
                        $data['is_applicable_discount'] = $is_applicable_discount;
                        $invoice_id = $this->student->insert('invoices', $data);
                        ...
                        $studentDetails = $this->student->get_single('students', array('id' => $student_id));
                        $ledger['registration_no'] = $studentDetails->registration_no;
                        $ledger['fee_due'] =  $this->input->post('previous_due');
                        ...
                        $this->student->insert('ledger',$ledger);
                    }
